...
def main():
    instance = Alertiq(config={
        "timeout": 30,
        "retries": 3,
        "debug": True,
    })

    print("=== Advanced alertiq Example ===\n")

    # Batch processing
    inputs = [f"data_{i}" for i in range(10)]
    results = [instance.manage(input=inp, index=i) for i, inp in enumerate(inputs)]
